...
const Auth: React.FC<AuthProps> = ({ onLogin }) => {
  ...
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMode('login');
      setStep(1);
      setSuccess('Cadastro criado com sucesso! Agora entre com seu e-mail e senha.');
    } catch (err: unknown) {
      const error = err as { code?: string; message?: string };
      const code = error.code ?? '';
  ...
  const FormSection = () => (
    <div className="p-4 sm:p-6 lg:p-10 flex flex-col">
      ...
      <div className="flex bg-slate-100 p-1 rounded-xl mb-6">
        <button
          type="button"
          onClick={() => { setMode('login'); setError(''); setSuccess(''); setShowForgotPassword(false); setForgotError(''); }}
        ...
        <button
          type="button"
          onClick={() => { setMode('register'); setError(''); setSuccess(''); setShowForgotPassword(false); }}
      ...
      {success && (
        <p className="text-sm text-green-700 bg-green-50 border border-green-200 px-4 py-2 rounded-xl mb-4">
          {success}
        </p>
      )}
